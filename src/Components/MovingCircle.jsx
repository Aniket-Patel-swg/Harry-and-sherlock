...
const MovingCircle = () => {
    const [position, setPosition] = useState({ x: Math.random() * 300, y: Math.random() * 400});
    ...
    useEffect(() => {
        if (moving) {
            const moveCircle = () => {
                ...
                const steps = 25; // Number of steps for the transition
                const stepDelay = 30; // Delay between each step in milliseconds

                // Calculate the increment for each step
                const xIncrement = (newX - position.x) / steps;
                const yIncrement = (newY - position.y) / steps;

                // Update the position gradually
                const updatePosition = (step) => {
                    const updatedX = position.x + xIncrement * step;
                    const updatedY = position.y + yIncrement * step;
                    setPosition({ x: updatedX, y: updatedY });
                };

                // Execute the steps with a delay between each step
                for (let step = 1; step <= steps; step++) {
                    setTimeout(() => updatePosition(step), step * stepDelay);
                }
                ...
                const animationInterval = setInterval(animateWings, 60);

                return () => {
                    clearInterval(animationInterval);
                };
            };
            const intervalId = setInterval(moveCircle, 700); // Change the interval as needed
    ...
    return (
        <>
            ...
            <svg
                ...
                id="snitch" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="208.89px" height="59.76px" viewBox="0 0 617.78 199.52" xml:space="preserve">
                <path
                 className={`wing ${rotateLeft ? 'rotate-left' : 'rotate-right'}`}
                id="leftwing" class="st0" d="M228.02,97.17c-9.21,4.19-12.49,3.48-18.36-3.98v0c-5.88-7.47-5.34-17.87,1.63-31.34
                    ...
                    C238.3,84.28,235.9,93.58,228.02,97.17" fill='#DAA621' />
                <path
                 className={`wing ${rotateLeft ? 'rotate-right' : 'rotate-left'}`}
                id="rightwing" class="st0" d="M385.24,98.25c-7.17-11.99,6.15-12.26,3.2-45.74c-0.64-20.72-0.09-24.48,4.48-30.67
                    ...
                    c-14.31-28.16,0.46,11.02,3.42,19.04c-5.03,7.83-13.8,18.34-23.01,13.58" fill='#DAA621' />
                <g id="body">
                    ...
                    <path class="st0" d="M274.24,190.69c-11.33-5.61-21.34-14.96-11.98-11.19c4.29,2.44,27.01,5.9,28.2,12.16
		                91.53,197.23,286.89,196.95,274.24,190.69L274.24,190.69z" fill='#DAA621' />
                    <path class="st0" d="M328.3,193.99c-2.46-8.09,26.08-15.81,31.7-14.55C361.49,179.97,330.64,201.33,328.3,193.99z" fill='#DAA621' />
                    <path class="st0" d="M270.85,179.57c-15.9-5.58-30.81-18.16-32.69-31.35c9,4.93,20.25,10.07,30.86,11.56
		c11.02,1.65,11.82,2.32,15.36,12.69C288.6,184.84,288.36,184.96,270.85,179.57z" fill='#DAA621' />
                    <path class="st0" d="M334.55,175.64c4.93-15.8,5.52-16.42,16.86-17.7c8.71-0.1,22.26-7.36,28.18-8.72
		c-3.74,14.55-17.41,28.07-37.28,30.61C333.04,181.51,332.77,181.36,334.55,175.64z" fill='#DAA621' />
		...
		c2.46-1.52,12.3,12.97,12.32,18.13C278.45,159.31,272.29,159.2,260.72,155.06L260.72,155.06z" fill='#DAA621' />
		...
		C367.34,90.59,354.74,91.44,347.96,87.65z" fill='#DAA621' />
            ...
            {/* <button onClick={handleClick} className="moving-circle" style={{ left: position.x, top: position.y }}></button>; */}
        </>
    )
};
